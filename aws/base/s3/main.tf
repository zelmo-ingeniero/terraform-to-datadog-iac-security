...
locals {
  object-lock-buckets = compact([for v in keys(var.buckets) :
    contains(var.buckets[v], "versioning") ? v : null
  ])
  logging-buckets = compact([for l in keys(var.buckets) :
    contains(var.buckets[l], "logging") ? l : null
  ])
}
...
#   bucket = each.value.id
#   rule {
#     # "ObjectWriter" or "BucketOwnerPreferred"
#     # default is BucketOwnerEnforced but with it the acls are not enabled
